...
export default async function Home() {
  ...
  // تصفية الأخبار حسب مكان الظهور (position) - إذا كان العمود موجوداً
  const featuredNews = allNews.filter(n => n.is_featured === true).slice(0, 4)
  const sideNews = allNews.filter(n => !n.position || n.position === 'auto').slice(0, 4)
  
  // أخبار القالب الأول
  const template1TopNews = allNews.filter(n => !n.position || n.position === 'auto').slice(0, 3)
  const template1LargeNews = allNews.filter(n => !n.position || n.position === 'auto').slice(3, 6)
  
  // أخبار القالب الثاني
  const template2News = allNews.filter(n => !n.position || n.position === 'auto').slice(6, 9)
  const template2LargeNews = allNews.filter(n => !n.position || n.position === 'auto').slice(9, 12)
  
  // أخبار القالب الثالث
  const template3News = allNews.filter(n => !n.position || n.position === 'auto').slice(12, 15)
  const template3LargeNews = allNews.filter(n => !n.position || n.position === 'auto').slice(15, 18)

  // أخبار عادية للقوائم الجانبية
  const regularNewsAll = allNews.filter(n => !n.position || n.position === 'auto')
