...
// ============================================================
const getEconomicAnalysisText = (locale: string) => {
  if (locale === 'en') return `Exclusive economic analysis provided by the Deep Source News team.`
  if (locale === 'ku') return `Analîzên aborî yên taybet ku ji hêla tîma Deep Source News ve têne pêşkêş kirin.`
  return `تحليلات اقتصادية حصرية يقدمها فريق ديب سورس نيوز.`
}

const getOurVisionText = (locale: string) => {
  if (locale === 'en') return `We strive to provide deep and independent news content that enhances awareness and supports knowledge.`
  if (locale === 'ku') return `Em hewl didin ku naverokeke nûçeyan a kûr û serbixwe pêşkêş bikin ku hişyariyê zêde dike û piştgiriyê zanînê dike.`
  return `نسعى لتقديم محتوى إخباري عميق ومستقل يعزز الوعي ويدعم المعرفة.`
}

const getZodiacText = (locale: string) => {
  if (locale === 'en') {
    return `🍀 Aries: A day full of positive energy and new opportunities.
🍀 Taurus: Upcoming financial opportunities and stability in relationships.
🍀 Gemini: Successful social communication and travel opportunities.
🍀 Cancer: Attention to family affairs and improvement in financial situations.
🍀 Leo: New professional achievements and appreciation from those around you.
🍀 Virgo: Health and mental focus and success in personal projects.`
  }
  if (locale === 'ku') {
    return `🍀 Beran: Rojek bi enerjiya erênî û firsendên nû.
🍀 Ga: Firsendên aborî yên tên û aramiya di têkiliyan de.
🍀 Cêwî: Danûstandina civakî ya serkeftî û firsendên rêwîtiyê.
🍀 Kevjal: Baldarî li karûbarên malbatê û başbûna rewşa aborî.
🍀 Şêr: Destkeftiyên pîşeyî yên nû û qedrê ji aliyê derdoran ve.
🍀 Zozan: Saxlemî û bala derûnî û serkeftina di projeyên kesane de.`
  }
  return `🍀 برج الحمل: يوم مليء بالطاقة الإيجابية والفرص الجديدة.
...
export default async function Home({ params }: PageProps) {
  ...
  // نصوص مترجمة
  const economicAnalysisText = getEconomicAnalysisText(locale)
  const ourVisionText = getOurVisionText(locale)
  const zodiacTodayText = getZodiacText(locale)

  // ترجمات العناوين حسب اللغة
  const t = {
    breakingNews: locale === 'en' ? 'Breaking News' : locale === 'ku' ? 'Nûçeyên Lezgîn' : 'أخبار عاجلة',
    economicAnalysis: locale === 'en' ? 'Economic Analysis' : locale === 'ku' ? 'Analîza Aborî' : 'التحليلات الاقتصادية',
    sideNews: locale === 'en' ? 'Side News' : locale === 'ku' ? 'Nûçeyên Alî' : 'أخبار جانبية',
    ourVision: locale === 'en' ? 'Our Vision' : locale === 'ku' ? 'Dîtina Me' : 'رؤيتنا',
    politicalAnalysis: locale === 'en' ? 'Political Analysis' : locale === 'ku' ? 'Analîza Siyasî' : 'تحليلات سياسية',
    zodiac: locale === 'en' ? 'Daily Zodiac' : locale === 'ku' ? 'Astroloja Rojane' : 'أبراج اليوم',
    news: locale === 'en' ? 'News' : locale === 'ku' ? 'Nûçe' : 'أخبار',
    globalPrices: locale === 'en' ? 'Global Prices' : locale === 'ku' ? 'Bihayê Cîhanî' : 'الأسعار العالمية',
    arabCurrencies: locale === 'en' ? 'Arab Currencies' : locale === 'ku' ? 'Diyarên Erebî' : 'العملات العربية',
    foreignCurrencies: locale === 'en' ? 'Foreign Currencies' : locale === 'ku' ? 'Diyarên Biyanî' : 'العملات الأجنبية',
    cryptoCurrencies: locale === 'en' ? 'Cryptocurrencies' : locale === 'ku' ? 'Diyarên Dîjîtal' : 'العملات الرقمية',
    preciousMetals: locale === 'en' ? 'Precious Metals' : locale === 'ku' ? 'Kanî yên Hêja' : 'المعادن النفيسة',
    energy: locale === 'en' ? 'Energy' : locale === 'ku' ? 'Enerjî' : 'الطاقة',
    indices: locale === 'en' ? 'Indices' : locale === 'ku' ? 'Endeks' : 'مؤشرات الأسهم',
    dailyBrief: locale === 'en' ? 'Daily Brief' : locale === 'ku' ? 'Rojnameya Rojane' : 'الجريدة اليومية',
    lastUpdate: locale === 'en' ? 'Last update' : locale === 'ku' ? 'Dawîn nûvekirin' : 'آخر تحديث',
  }

  const stats = [
    { icon: FaGlobe, value: '120+', label: locale === 'en' ? 'Countries' : locale === 'ku' ? 'Welat' : 'دولة' },
    { icon: FaUsers, value: '50K+', label: locale === 'en' ? 'Readers' : locale === 'ku' ? 'Xwendevan' : 'قارئ' },
    { icon: FaChartLine, value: '10K+', label: locale === 'en' ? 'Articles' : locale === 'ku' ? 'Gotar' : 'مقال' },
    { icon: FaCalendarAlt, value: 'Since 2024', label: locale === 'en' ? 'Launch' : locale === 'ku' ? 'Destpêk' : 'الانطلاق' },
  ]

  const NumberedListWithImage = ({ items, color = "text-red-600" }: any) => (
    <div className="space-y-2">
      {items.map((news: any, idx: number) => (
        <a key={news.id} href={`/${locale}/news/${news.slug}`} className="flex gap-2 group hover:bg-red-50 dark:hover:bg-red-950/30 p-1.5 rounded-lg transition">
          ...
          <div className="flex-1">
            ...
            <span className="text-gray-400 text-[10px]">{new Date(news.created_at).toLocaleDateString(locale === 'en' ? 'en-US' : 'ar-EG')}</span>
          </div>
        </a>
      ))}
    </div>
        ...
        <section className="relative bg-cover bg-center bg-no-repeat" style={{ backgroundImage: `url(${hero.background_image})` }}>
          ...
          <div className="relative z-10 container-custom text-center py-16 md:py-24">
            ...
            {hero.button_text && hero.button_link && (
              <a href={`/${locale}${hero.button_link === '/' ? '' : hero.button_link}`} className="inline-block bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-6 rounded-full transition-all duration-300 hover:scale-105 text-sm">{hero.button_text}</a>
            )}
          </div>
        </section>
      )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          ...
          <div className="lg:col-span-3 order-2 lg:order-1">
            <div className="bg-white/50 dark:bg-gray-800/50 p-3 rounded-xl">
              ...
              <FreeTextBlock title={t.economicAnalysis} icon={<FaChartBar size={10} />} content={economicAnalysisText} color="text-emerald-600 dark:text-emerald-400" />
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-4">
              {smallGroups[0].map((news) => (
                <div key={news.id} className="bg-white/50 dark:bg-gray-800/50 p-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/30 transition group">
                  <a href={`/${locale}/news/${news.slug}`} className="flex gap-2">
                    {news.image && <img src={news.image} alt={news.title} className="w-14 h-14 object-cover rounded" />}
                    <div className="flex-1">
                      <span className="text-red-500 text-[10px] font-bold">{news.category || (locale === 'en' ? 'News' : locale === 'ku' ? 'Nûçe' : 'أخبار')}</span>
                ...
                <div key={news.id} className="bg-white/50 dark:bg-gray-800/50 rounded-lg overflow-hidden hover:bg-red-50 dark:hover:bg-red-950/30 transition group">
                  <a href={`/${locale}/news/${news.slug}`}>
                    ...
                    <div className="p-3">
                      ...
                      <span className="text-gray-400 text-[10px] mt-1 block">{new Date(news.created_at).toLocaleDateString(locale === 'en' ? 'en-US' : 'ar-EG')}</span>
                    </div>
                  </a>
                </div>
              ))}
          ...
          <div className="lg:col-span-3 order-3">
            <div className="bg-white/50 dark:bg-gray-800/50 p-3 rounded-xl">
              ...
              <FreeTextBlock title={t.ourVision} icon={<FaEye size={10} />} content={ourVisionText} color="text-purple-600 dark:text-purple-400" />
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="lg:col-span-6 order-1 lg:order-2">
              {mainNews && (
                <div className="bg-white/50 dark:bg-gray-800/50 rounded-xl overflow-hidden hover:bg-red-50 dark:hover:bg-red-950/30 transition group">
                  <a href={`/${locale}/news/${mainNews.slug}`}>
                    {mainNews.image && <img src={mainNews.image} alt={mainNews.title} className="w-full h-72 object-cover" />}
                    <div className="p-4">
                      <span className="text-red-500 text-xs font-bold">{mainNews.category || (locale === 'en' ? 'News' : locale === 'ku' ? 'Nûçe' : 'أخبار')}</span>
                      ...
                      <span className="text-gray-400 text-xs mt-2 block">{new Date(mainNews.created_at).toLocaleDateString(locale === 'en' ? 'en-US' : 'ar-EG')}</span>
                    </div>
                  </a>
                </div>
              )}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden border border-gray-100 dark:border-gray-700">
              ...
              <div className="p-3 max-h-[550px] overflow-y-auto">
                ...
                <div className="mt-3 pt-2 text-center border-t border-gray-200 dark:border-gray-700">
                  <p className="text-[9px] text-gray-400">{t.lastUpdate}: {new Date().toLocaleTimeString(locale === 'en' ? 'en-US' : 'ar-EG')}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="bg-gray-900 text-white mt-6">
        <div className="container-custom py-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2 text-sm text-gray-400"><FaCalendarAlt /><span>{new Date().toLocaleDateString(locale === 'en' ? 'en-US' : 'ar-EG')}</span></div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {dailyBrief.length > 0 ? dailyBrief.map((item, idx) => (
              ...
              <>
                <div className="bg-gray-800/50 p-3 hover:bg-gray-800 transition rounded-lg cursor-pointer"><p className="text-orange-400 text-xs font-bold mb-1">{locale === 'en' ? 'Main Title' : locale === 'ku' ? 'Sernavê Sereke' : 'العنوان الرئيسي'}</p><p className="text-white text-xs">{locale === 'en' ? 'Technological Developments' : locale === 'ku' ? 'Pêşketinên Teknolojîk' : 'التطورات التكنولوجية'}</p></div>
                <div className="bg-gray-800/50 p-3 hover:bg-gray-800 transition rounded-lg cursor-pointer"><p className="text-blue-400 text-xs font-bold mb-1">{locale === 'en' ? 'Economy Today' : locale === 'ku' ? 'Aboriya Îro' : 'الاقتصاد اليوم'}</p><p className="text-white text-xs">{locale === 'en' ? 'Oil Prices Rise' : locale === 'ku' ? 'Bihayê Neftê Bilind Dibin' : 'أسعار النفط ترتفع'}</p></div>
                <div className="bg-gray-800/50 p-3 hover:bg-gray-800 transition rounded-lg cursor-pointer"><p className="text-green-400 text-xs font-bold mb-1">{locale === 'en' ? 'Technology' : locale === 'ku' ? 'Teknolojî' : 'التكنولوجيا'}</p><p className="text-white text-xs">{locale === 'en' ? 'AI Revolution' : locale === 'ku' ? 'Şoreşa AI' : 'ثورة الذكاء الاصطناعي'}</p></div>
                <div className="bg-gray-800/50 p-3 hover:bg-gray-800 transition rounded-lg cursor-pointer"><p className="text-purple-400 text-xs font-bold mb-1">{locale === 'en' ? 'Sports' : locale === 'ku' ? 'Werzîş' : 'الرياضة'}</p><p className="text-white text-xs">{locale === 'en' ? 'Final Match Preparations' : locale === 'ku' ? 'Amadekirinên Maça Finalê' : 'استعدادات المباراة النهائية'}</p></div>
              </>
            )}
          </div>
        </div>
